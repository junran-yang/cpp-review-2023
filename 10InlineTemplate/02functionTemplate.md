...
## a shorted list of compilation summary
* compilation
    * (note to self) a template for compiler to generate function code
    ...
    2. semantic analysis    
        * function resolution
            * name lookup       => error
                - template world vs non-template world, depending on call site
            * type deduction    => error
                - `<T, T>` with `(1.2, 1)` will fail
            * choose best
                - prefer non-template if equally best
        * function generation
            - it's okay if no function is generated, as long as we find a symbol
            def in linking stage
            * ODR
                - should be put in header; ODR same as inline; 
                - generated once per TU
            * duck typing       => error
                - generic code thinking methodology
